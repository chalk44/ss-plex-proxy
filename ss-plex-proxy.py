...
def setup_logging():
    ...
    for logger in (app.logger, logging.getLogger("pysmoothstreams")):
...
def get_channel(channel_number):
    url = guide.build_stream_url(
        Server[server], channel_number, auth_sign, protocol=Protocol.MPEG
    )
    return redirect(url)
...
def guide_data():
    ...
    if app.config["NANOF_LOGOS"]:
        epg_data = replace_logos(epg_data)
    ...
    for element in tree.iter():
        if element.tag == "channel":
            ...
            if os.path.isfile(logo_path):
                element.find("icon").attrib["src"] = (
                    request.url_root + "logos/" + channel_name + ".png"
                )
...
def generate_m3u_playlist():
    ...
    for channel in guide.channels:
        ...
        m3u += "{url_root}channels/{channel_number}\n".format(
            url_root=request.url_root, channel_number=str(channel["number"])
        )
